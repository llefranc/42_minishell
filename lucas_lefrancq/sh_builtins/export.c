...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/10/06 12:28:51 by lucas          #+#    #+#             */
/*   Updated: 2020/10/26 18:13:17 by lucas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
** Allocates a new string with and double the backslashs. Frees the previous
** string.
*/
int		double_the_backslash(char **tmp_env, int i)
{
	int		size;
	int		j;
	int		nb_backslash;
	...
	nb_backslash = 0;
	size = ft_strlen(tmp_env[i]);
	j = -1;
	while (tmp_env[i][++j])
		tmp_env[i][j] == '\\' ? nb_backslash++ : 0; //counting nb of backshlash
	if (!(tmp = malloc(size + nb_backslash + 1)))
		return (1);
	ft_memset(tmp, 0, size + nb_backslash + 1);
	ft_strlcpy(tmp, tmp_env[i], size + 1);
	j = -1;
	while (tmp[++j])
	{
		if (tmp[j] == '\\')
		{
			ft_memmove(tmp + j + 1, tmp + j, ft_strlen(tmp + j + 1) + 1); //double each backslash
			j++;
		}
	}
	free(tmp_env[i]);
...
/*
** Sorts the tmp_env array and prints it on stdout. Doubles the backslashs.
** Then free the tmp_env.
*/
int	    print_sort_env(char **tmp_env)
{
    int     i;
    char    *tmp_swap;
	...
	{
		if (double_the_backslash(tmp_env, i))
			return (error_msg("export: malloc failed\n", FAILURE));
        ft_fd_printf(STDOUT_FILENO, "declare -x %s\n", tmp_env[i++]);
	}
    free_split(tmp_env);
	return (SUCCESS);
}
...
int     builtin_export(char **args, char ***env)
{
    ...
    if (args && !args[1]) //if no arguments and only export cmd >> prints env
        return (print_sort_env(copy_env(*env, 1)));
    if (args[1][0] == '-') //our export doesn't handle options
        return (error_msg("export: no options are allowed\n", FAILURE));
    i = 0;
